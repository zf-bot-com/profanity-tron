...
![](https://img.shields.io/github/actions/workflow/status/zf-bot-com/profanity-tron/release.yml)
...
![](https://img.shields.io/badge/telegram-@zf_talk-blue.svg)
...
需要以太坊（ETH）地址生成的，请前往：[profanity-ether](https://github.com/zf-bot-com/profanity-ether)

## 说明

- 本程序基于以太坊地址生成器：[profanity](https://github.com/zf-bot-com/profanity-ether) 修改而来，同时修复了原程序私钥可爆破的问题。请参考下方 `安全` 章节说明。
...
前往 [Release](https://github.com/zf-bot-com/profanity-tron/releases) 页面下载发布包（window.zip），本地解压后直接运行 `start.bat`。
...
如果提示 “Apple无法验证“profanity.x64”是否包含可能危害Mac安全或泄漏隐私的恶意软件。”, 点击 “完成”，然后：

1. 打开 Mac 的 “系统设置” (System Settings)。
2. 点击左侧边栏的 “隐私与安全性” (Privacy & Security)。
3. 向下滑动找到 “安全性” (Security) 栏目。
4. 你会看到一条提示：“由于无法验证开发者，‘profanity.x64’被阻止使用”。
5. 点击旁边的 “仍要打开” (Open Anyway)，然后输入你的开机密码。
6. 再次运行程序，弹窗会多出一个“打开”按钮，点击即可。
...
Examples:
  ...
  ./profanity --matching TUqEg3dzVEJNQSVW2HY98z5X8SBdh11111 --suffix-count 5 --quit-count 1
...
About:
  ...
  Please make sure the program you are running is download from: https://github.com/zf-bot-com/profanity-tron
  Author: telegram -> @zf_talk
...
```bash
.\profanity.exe --skip 1 --matching TP9kwGiEGaqoGo8AZ9QZhLa8G7iStdXSk7 --prefix-count 4 --suffix-count 5
```
